...
(function sortableModule(factory) {
	...
	var dragEl,
		...
		_isClientInRowColumn = function(x, y, el, axis, options) {
			var targetRect = _getRect(el),
				...
				targetS2Opp =
					axis === "vertical" ? targetRect.right : targetRect.bottom,
		...
		_autoScroll = _throttle(function(
			...
			if (options.scroll) {
				...
				do {
					...
					vx =
						canScrollX &&
						(abs(right - x) <= sens && scrollPosX + width < scrollWidth) -
							(abs(left - x) <= sens && !!scrollPosX);

					vy =
						canScrollY &&
						(abs(bottom - y) <= sens && scrollPosY + height < scrollHeight) -
							(abs(top - y) <= sens && !!scrollPosY);
					...
					(currentParent = _getParentAutoScrollElement(currentParent, false))
				);
	...
	Sortable.prototype = /** @lends Sortable.prototype */ {
		...
		_computeIsAligned: function(evt) {
			...
			for (var i = 0; i < children.length; i++) {
				...
				if (
					_closest(children[i], this.options.draggable, this.el, false) &&
					children[i] !== target
				) {
					children[i].sortableMouseAligned = _isClientInRowColumn(
						evt.clientX,
						evt.clientY,
						children[i],
						this._getDirection(evt, null),
						this.options
					);
				}
			}
		...
		_onTapStart: function(/** Event|TouchEvent */ evt) {
			...
			if (
				options.handle &&
				!_closest(originalTarget, options.handle, el, false)
			) {
				return;
			}

			// Prepare `dragstart`
			this._prepareDragStart(
				evt,
				touch,
				target,
				startIndex,
		...
		_handleAutoScroll: function(evt, fallback) {
			...
			if (fallback || Edge || IE11OrLess || Safari) {
				...
				if (
					!_this.options.bubbleScroll ||
					_getParentAutoScrollElement(elem, true) ===
				...
				_autoScroll(
					evt,
					_this.options,
					_getParentAutoScrollElement(elem, false),
					false
				);
			}
		},
		...
		_prepareDragStart: function(
			...
			if (target && !dragEl && target.parentNode === el) {
				...
				if (
					options.delay &&
					(options.delayOnTouchOnly
						? evt.pointerType === "touch" || evt.pointerType === "pen"
							? true
							: false
						: true) &&
			...
			if (tapEvt) {
				var options = this.options,
					...
					relativeScrollOffset =
						...
						ghostRelativeParent &&
						_getRelativeScrollOffset(ghostRelativeParent),
					dx =
		...
		_appendGhost: function() {
			...
			if (!ghostEl) {
				...
				if (PositionGhostAbsolutely) {
					...
					ghostRelativeParentInitialScroll = _getRelativeScrollOffset(
						ghostRelativeParent
					);
				}
		...
		_onDragOver: function(/**Event*/ evt) {
			...
			function changed() {
				_dispatchEvent(
					...
					_index(dragEl),
					oldDraggableIndex,
					_index(dragEl, options.draggable),
					evt
				);
			}
			...
			if (
				...
				if (
					!elLastChild ||
					(_ghostIsLast(evt, axis, el) && !elLastChild.animated)
				) {
					...
					var direction = 0,
						...
						scrolledPastTop =
							_isScrolledPast(target, "top") || _isScrolledPast(dragEl, "top"),
			...
			if (ms) {
				...
				if (
					...
					_css(
						...
						"transform",
						"translate3d(" +
							(prevRect.left - currentRect.left) / (scaleX ? scaleX : 1) +
							"px," +
	...
	function _ghostIsLast(evt, axis, el) {
		...
		return axis === "vertical"
			? mouseOnOppAxis > targetS2Opp + spacer ||
					(mouseOnOppAxis <= targetS2Opp &&
						mouseOnAxis > targetS2 &&
						mouseOnOppAxis >= targetS1Opp)
			: (mouseOnAxis > targetS2 && mouseOnOppAxis > targetS1Opp) ||
					(mouseOnAxis <= targetS2 && mouseOnOppAxis > targetS2Opp + spacer);
	}
			...
			if (!IE11OrLess) {
				do {
					if (
						...
						top -=
							containerRect.top + parseInt(_css(container, "border-top-width"));
						left -=
							containerRect.left +
							parseInt(_css(container, "border-left-width"));
