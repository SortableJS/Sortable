...
fixture`Simple Sorting`.page`./fixtures/single-list.html`;
...
fixture`Grouping`.page`./fixtures/dual-list.html`;
...
test('Clone element if pull:"clone"', async (browser) => {
	...
	await browser
		...
		.dragToElement(dragEl, target, { offsetY: 0, destinationOffsetY: 0 })
		.expect(dragStartPosition.innerText)
		.eql(dragEl.innerText) // clone check
		.expect(dragEndPosition.innerText)
		.eql(dragEl.innerText)
		.expect(targetEndPosition.innerText)
		.eql(target.innerText);
...
fixture`Handles`.page`./fixtures/handles.html`;
...
fixture`Filter`.page`./fixtures/filter.html`;
...
fixture`Nested`.page`./fixtures/nested.html`;
...
fixture`Empty Insert`.page`./fixtures/empty-list.html`;

test("Insert into empty list if within emptyInsertThreshold", async (browser) => {
	const threshold = await browser.eval(() =>
	...
	await browser
		...
		.drag(
			dragEl,
			Math.round(list2Rect.left - dragRect.left) - (threshold - 1),
			-(threshold - 1),
			{
				offsetY: 0,
				offsetX: 0,
